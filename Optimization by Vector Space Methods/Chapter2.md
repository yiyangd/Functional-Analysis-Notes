...
A necessary and sufficient condition for the set of vectors $x_1, x_2, \ldots, x_n$ to be linearly independent is that the expression $\sum_{k=1}^{n} \alpha_k x_k = \theta$ implies $\alpha_k = 0$ for all $k = 1, 2, 3, \ldots, n$.  
向量集 $x_1, x_2, \ldots, x_n $ 线性独立的必要充分条件是，表达式 $\sum_{k=1}^{n} \alpha_k x_k = \theta$ 意味着对于所有的 $k = 1, 2, 3, \ldots, n$，有 $\alpha_k = 0$。

#### 5.4. Unique Expression
If $x_1, x_2, \ldots, x_n$ are linearly independent vectors, and $\sum_{k=1}^{n} \alpha_k x_k = \sum_{k=1}^{n} \beta_k x_k$, then $\alpha_k = \beta_k$ for all $k = 1, 2, \ldots, n$.  
如果 $x_1, x_2, \ldots, x_n$ 是线性独立的向量，且 $\sum_{k=1}^{n} \alpha_k x_k = \sum_{k=1}^{n} \beta_k x_k$ 那么对于所有的 $k = 1, 2, \ldots, n$，有 $\alpha_k = \beta_k$。

**Proof:**  
- if $\sum_{k=1}^{n} \alpha_k x_k = \sum_{k=1}^{n} \beta_k x_k$, then $\sum^n_{i=1}(\alpha_k - \beta_k)x_k = \theta$
- according to the Theorem of 5.3, since $\{x_i\}$ are linearly independent, $(\alpha_k-\beta_k)= 0$ for all $k = 1, 2, 3, \ldots, n$

### 6. Finite and Infinite Dimension
#### 6.1. Definition
A **finite set S** of *linearly independent vectors* is said to be a **basis** for the space X **if S generates X**. 
如果*线性独立向量*的**有限集合 S 生成了空间 X**，则称 **S 为 X 的一组基**。
- A vector space having a **finite basis** is said to be **finite dimensional**.   
拥有**有限基**的向量空间称为**有限维的向量空间**
- **All other** vector spaces are said to be **infinite dimensional**.  
**所有其他**向量空间被称为**无限维的向量空间**。
#### 6.2. Theorem
Any two bases for a finite-dimensional vector space contain the same number of elements.  
任何两个有限维向量空间的基都包含相同数量的元素。
- A space with **a basis consisting of n elements** is referred to as an **n-dimensional space**.  

#### 6.3. Finite vs Infinite
Finite-dimensional spaces are somewhat simpler to analyze than infinite-dimensional spaces.     
有限维空间比无限维空间分析起来要简单一些。
- Fewer definitions are required, fewer cases arise, and our native intuition is contradicted in fewer instances in finite-dimensional spaces.  
需要的定义更少，出现的问题情况更少，而且在有限维空间中，我们的直观判断更少地被违背。

Furthermore, there are theorems and concepts in finite-dimensional space which have no direct counterpart in infinite-dimensional spaces.  
此外，有限维空间中存在的定理和概念在无限维空间中没有直接对应物。
- however, that results first derived for finite-dimensional spaces do have direct analogs in more general spaces.   
然而，最初在有限维空间推导出的结果在更一般的空间中确实有直接的类比。
- In these cases, verification of the corresponding result in infinite-dimensional space often enhances our understanding by indicating precisely which properties of the space are responsible for the result.   
在这种情况下，在无穷维空间中验证相应的结果，往往能准确地指出是空间的哪些属性导致了这一结果，从而加深我们的理解。
- We constantly endeavor to stress the similarities between infinite- and finite-dimensional spaces rather than the few minor differences.  
我们不断努力强调无限维和有限维空间之间的相似性，而不是少数细微差异。
